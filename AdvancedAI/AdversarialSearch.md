...
在 3×3 的棋盘上，如果有9个空位，需要预测3步，则需要展开节点数为 $1+9 + 9\times8 + 9\times8\times7=586$，如果是 8×8 的棋盘，如果有64 个空位，需要预测3步，则需要展开节点数为 $1+64+64\times63+64\times63\times62=254081$。n×n 的棋盘，需要预测 k 步，一步决策的 时间复杂度 大约为 $O(n^{2k})$ 次，也是指数级的增长，这样的开销完全不适用，不过该算法仍然可以作为对博弈进行数学分析和设计实用算法的基础。因为在程序中一次性生成所有的后继，并且在判断后及时清理，因此该算法的空间复杂度为 $O(n^2k)$，空间复杂度并不大。